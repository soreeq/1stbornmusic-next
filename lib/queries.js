export const beatsQuery = `
  *[_type == "beat" && visibility == "public"] | order(plays desc) {
    ...
    status, isNew, isFeatured, plays, prices,
...
export const vaultBeatsQuery = `
  *[_type == "beat" && visibility == "vault"] | order(plays desc) {
    _id, title, slug, bpm, key, tags,
    "audioSrc": select(
      defined(audioFile.asset) => audioFile.asset->url,
      audioSrc
...
export const collectionsQuery = `
  *[_type == "collection" && isVisible == true] | order(order asc) {
    _id, title, slug, description, "coverImage": coverImage.asset->url, youtubeId, order,
    "beats": select(
      count(beats) > 0 => beats[@->visibility == "public"]->{
      ...
      *[_type == "beat" && references(^._id) && visibility == "public"] | order(plays desc) {
        _id, title, bpm, key, tags,
        "audioSrc": select(
          defined(audioFile.asset) => audioFile.asset->url,
          audioSrc
